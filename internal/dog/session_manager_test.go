...
// TestDogStartup_SleepForReadyDelay verifies that SleepForReadyDelay is called
// after WaitForCommand in dog startup when ReadyDelayMs is configured.
func TestDogStartup_SleepForReadyDelay(t *testing.T) {
	// This test verifies the timing behavior in Start() method
	// The key assertion is that SleepForReadyDelay is called after WaitForCommand
	// and respects the configured ReadyDelayMs value

	tmpDir := t.TempDir()
	dogName := "test-dog"

	// Create the kennel directory structure
	dogDir := filepath.Join(tmpDir, "deacon", "dogs", dogName)
	...
	// Write runtime config to dog directory
	settingsPath := filepath.Join(dogDir, ".settings.json")
...
// TestDogStartup_FallbackNudgeMatrix verifies GetStartupFallbackInfo is called
// and the nudge matrix is correctly applied based on agent capabilities.
func TestDogStartup_FallbackNudgeMatrix(t *testing.T) {
	tests := []struct {
		name                    string
		provider                string
...
// TestDogStartup_CombinedNudgeCase verifies the combined nudge case where
// beacon and startup nudges are sent together (when delay=0 for hook agents without prompt).
func TestDogStartup_CombinedNudgeCase(t *testing.T) {
...
// TestDogStartup_SeparateNudgeCase verifies the separate nudge case where
// beacon is sent first, then delayed startup nudge (for non-hook agents).
func TestDogStartup_SeparateNudgeCase(t *testing.T) {
...
// TestDogStartup_LegacyFallbackPreserved verifies that RunStartupFallback
// still works as final fallback when hooks are not configured.
func TestDogStartup_LegacyFallbackPreserved(t *testing.T) {
	tmpDir := t.TempDir()
	dogName := "test-dog-legacy"
...
// TestDogSessionManagerStart_ConfigPropagation verifies that runtime config
// is properly propagated through the startup sequence.
func TestDogSessionManagerStart_ConfigPropagation(t *testing.T) {
	tmpDir := t.TempDir()
	townName := "test-town"
	...
	expectedSessionName := "gt-test-town-deacon-test-dog"
	actualSessionName := sm.SessionName(dogName)
	if actualSessionName != expectedSessionName {
		t.Errorf("SessionName: got %q, want %q", actualSessionName, expectedSessionName)
	}

	// Verify kennel path
	expectedKennelPath := filepath.Join(tmpDir, "deacon", "dogs", dogName)
	actualKennelPath := sm.kennelPath(dogName)
	if actualKennelPath != expectedKennelPath {
		t.Errorf("kennelPath: got %q, want %q", actualKennelPath, expectedKennelPath)
	}
}

// TestDogStartup_WithDelayTiming verifies the timing behavior of SleepForReadyDelay
// in the context of dog startup.
func TestDogStartup_WithDelayTiming(t *testing.T) {
	tests := []struct {
		name         string
		readyDelayMs int
...
// TestDogStartup_WithoutDelay verifies that startup proceeds normally
// when ReadyDelayMs is 0 or not configured.
func TestDogStartup_WithoutDelay(t *testing.T) {
	tests := []struct {
		name string
		rc   *config.RuntimeConfig
	}{
